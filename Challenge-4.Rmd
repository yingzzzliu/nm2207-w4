...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data%>%filter(sentiment>0)%>%group_by(sender)%>%summarise(average_sentiment=mean(sentiment))%>%arrange(desc(average_sentiment))%>%slice(1:3)
```
<br>
...
```{r, eval=FALSE,echo=TRUE}
# Enter code here
comm_data%>%mutate(sentiment_label= case_when(sentiment>0 ~ "Positive",sentiment<0 ~ "Negative", TRUE ~ "Neutral"))%>%select(date,channel,sender,message,sentiment_label)
```
<br>
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
comm_data%>%group_by(date)%>%summarise(total_characters = sum(nchar(message)))%>%arrange(desc(total_characters))%>%slice(1:1)
```
<br>
